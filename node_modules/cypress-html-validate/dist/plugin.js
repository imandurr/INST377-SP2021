...
on, userConfig, userOptions) {
    ...
    const options = Object.assign(Object.assign({}, options_1.default), userOptions);
    if (!Array.isArray(options.exclude)) {
        throw new Error(`Invalid cypress-html-validate configuration, "exclude" must be array, got ${typeof options.exclude}`);
    }
    if (!Array.isArray(options.include)) {
        throw new Error(`Invalid cypress-html-validate configuration, "include" must be array, got ${typeof options.include}`);
    }
    on("task", {
        htmlvalidate(source) {
